...
def volgen_fix(
    ...
    while True:
        ...
        indices = np.random.randint(n_paired, size=batch_size) * 2 + 1
...
def volgen_paired(
    ...
    while True:
        ...
        # indices = np.arange(start_index,start_index+batch_size)
        # indices = np.random.randint(len(vol_names), size=batch_size)
        index = np.random.randint(n_paired, size=batch_size)
        indices_mov = index * 2
        indices_fix = index * 2 + 1

        # load volumes and concatenate
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape,
                           resize_factor=resize_factor)
        imgs_fix = [utils.load_volfile(vol_names[i], **load_params) for i in indices_fix]
        imgs_mov = [utils.load_volfile(vol_names[i], **load_params) for i in indices_mov]
        ...
        if return_segs:
            load_params['np_var'] = 'seg'  # be sure to load seg
            segs_mov = [utils.load_volfile(vol_names[i], **load_params) for i in indices_mov]
            segs_fix = [utils.load_volfile(vol_names[i], **load_params) for i in indices_fix]
...
def volgen_paried_with_seg(
    ...
    while True:
        ...
        index = np.random.randint(n_paired, size=batch_size)
        indices_mov = index * 9
        indices_fix = index * 9 + 2
        indices_mov_mask = index * 9 + 1
        indices_fix_mask = index * 9 + 3
        ...
        if return_segs:
            load_params['np_var'] = 'seg'  # be sure to load seg
            segs_mov = [utils.load_volfile(vol_names[i], **load_params) for i in indices_mov_mask]
            segs_fix = [utils.load_volfile(vol_names[i], **load_params) for i in indices_fix_mask]
...
def volgen_paried_randpatch(
    ...
    while True:
        ...
        index_mov = index[0] * 9
        index_fix = index[0] * 9 + 2
        index_mov_mask = index[0] * 9 + 1
        index_fix_mask = index[0] * 9 + 3
        index_mask_mov = index[0] * 9 + 4
        index_mask_fix = index[0] * 9 + 5
        index_artery = index[0] * 9 + 6
        index_vein = index[0] * 9 + 7
        index_protal_vein = index[0] * 9 + 8
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape,
                           resize_factor=resize_factor)
        imgs_fix = utils.load_volfile(vol_names[index_fix], **load_params)
        imgs_mov = utils.load_volfile(vol_names[index_mov], **load_params)
        full_size = imgs_fix.shape
        col_idx = np.random.randint(0, full_size[2] - 80, batch_size)
        row_idx = np.random.randint(0, full_size[1] - 80, batch_size)
        sl_idx = np.random.randint(0, full_size[3] - 80, batch_size)
        imgs_fix_patches = [imgs_fix[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                     80, 80, 80, 1) for i in range(batch_size)]
        imgs_mov_patches = [imgs_mov[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                     80, 80, 80, 1) for i in range(batch_size)]
        vols_mov = [np.concatenate(imgs_mov_patches, axis=0)]
        vols_fix = [np.concatenate(imgs_fix_patches, axis=0)]


        imgs_mov_mask = utils.load_volfile(vol_names[index_mask_mov], **load_params)
        imgs_fix_mask = utils.load_volfile(vol_names[index_mask_fix], **load_params)
        ...
        imgs_portal_vein = utils.load_volfile(vol_names[index_protal_vein], **load_params)
        imgs_mov_mask_patches = [
            imgs_mov_mask[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     1)
            for i in range(batch_size)]
        imgs_fix_mask_patches = [
            imgs_fix_mask[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     1)
            for i in range(batch_size)]
        imgs_artery_patches = [
            imgs_artery[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     1)
            for i in range(batch_size)]
        imgs_vein_patches = [
            imgs_vein[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     1)
            for i in range(batch_size)]
        imgs_portal_vein_patches = [
            imgs_portal_vein[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80, 0].reshape(1,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     80,
                                                                                                                     1)
        ...
        if return_segs:
            load_params['np_var'] = 'seg'  # be sure to load seg
            segs_mov = utils.load_volfile(vol_names[index_mov_mask], **load_params)
            segs_fix = utils.load_volfile(vol_names[index_fix_mask], **load_params)
            imgs_fix_seg_patches = [
                segs_fix[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80,
                0].reshape(1, 80, 80, 80, 1) for i
                in range(batch_size)]
            imgs_mov_seg_patches = [
                segs_mov[0, row_idx[i]:row_idx[i] + 80, col_idx[i]:col_idx[i] + 80, sl_idx[i]:sl_idx[i] + 80,
                0].reshape(1, 80, 80, 80, 1) for i
                in range(batch_size)]

            vols_mov.append(np.concatenate(imgs_mov_seg_patches, axis=0))
            vols_mov.append(np.concatenate(imgs_mov_mask_patches, axis=0))
            vols_fix.append(np.concatenate(imgs_fix_seg_patches, axis=0))
            vols_fix.append(np.concatenate(imgs_fix_mask_patches, axis=0))
...
def scan_to_scan_norand(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
...
def scan_to_scan_norand_mask(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    n_paired = (len(vol_names) / 4)
    # gen_paired = volgen_paired(vol_names, int(n_paired), batch_size=batch_size, **kwargs)
    gen_paired = volgen_paried_with_seg(vol_names, int(n_paired), batch_size=batch_size, **kwargs)
    ...
    while True:
        ...
        # invols = [src_vol, trg_vol, src_seg]
        # outvols = [trg_vol, zeros, trg_seg]
        invols = [scan1, scan2, scan1_seg, scan2_seg]
        outvols = [scan2, scan1] if bidir else [scan2]
        if not no_warp:
            outvols.append(zeros)
...
def surf_semisupervised(
    ...
    std_to_surf = lambda x, y: utils.sdt_to_surface_pts(x, y,
